...
`apps/web/vercel.json` carries an `ignoreCommand` running `scripts/vercel-ignore-build.sh`, which exits 0 to skip a deployment and 1 to build it. Vercel bills Build CPU Minutes per deployment and the charge is dominated by fixed per-deploy overhead — container provision, install, upload — not by the build step, which runs in 21-31s here; a month of deploys billed roughly 8 CPU-minutes each. Skipping a commit that changes nothing deployable therefore saves the whole deploy, while making a fast build faster saves almost nothing. The predicate is one-sided on purpose: it skips only when EVERY changed path is `docs/`, `.github/`, `infrastructure/`, `artifacts/`, `tools/`, or `*.md`, and a deployable path, an unreadable diff, a missing parent commit, or an empty file list all build. A wrong skip ships stale code and is invisible; a wrong build costs a fraction of a cent. `scripts/vercel-ignore-build-test.sh` runs ten cases against real git repositories and ends by deleting the catch-all that marks unknown paths deployable, confirming the deletion landed, and proving the mutant then skips a source change. `neon-branch-cleanup.yml`: deletes this pull request's Neon preview database when the pull request closes, and sweeps weekly for any it missed. Neon's Vercel integration removes a `preview/<git-branch>` database only when the Vercel *deployment* is removed — six months by default — so merging a pull request and deleting the git branch leaves the database billing at about $1.50/month; this repo reached 93 branches and $79.60 of an $81.78 invoice that way (2026-09-03), against $2.18 of real compute and storage. It runs on `pull_request_target`, not `pull_request`, because a Dependabot-authored `pull_request` run reads only Dependabot secrets and would resolve `NEON_API_KEY` to an empty string, reaping nothing on a large share of pull requests; it never checks out or executes pull request code. It needs the `NEON_API_KEY` secret and the `NEON_PROJECT_ID` variable, exits 78 with one but not the other, and gates nothing. `scripts/neon_branches.py` is the same logic as a local tool, dry-run by default.
