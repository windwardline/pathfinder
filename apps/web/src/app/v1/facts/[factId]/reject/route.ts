...
export async function POST(request: Request, context: { params: Promise<{ factId: string }> }) {
  ...
  if (!parsed.success) {
    return apiError({ status: 400, code: 'INVALID_REQUEST', message: 'Invalid rejection request', details: parsed.error.flatten().fieldErrors, correlationId: correlationId(request) });
  }
